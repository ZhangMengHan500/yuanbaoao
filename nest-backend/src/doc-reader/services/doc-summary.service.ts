...
export class DocSummaryService {
  ...
  async generateSummaries(docId: string, chunks: DocChunk[]) {
  ...
  private async generateExecutiveSummary(text: string): Promise<string> {
  ...
  private async generateKeyPoints(text: string): Promise<string[]> {
    const inputText = text.substring(0, 3000);
  ...
  private async generateOutline(chunks: DocChunk[]): Promise<string> {
    const titles = chunks
      .map(c => c.metadata.title)
      .filter(t => t && t.length > 0);

    if (titles.length > 0) {
      // 清理标题中自带的编号前缀，避免大纲出现 "1. 1. xxx"
    ...
    // 兜底清理：即使 LLM 返回了 markdown 标记也去掉
    return result
      .replace(/^#{1,6}\s+/gm, '')
      .replace(/\*\*(.*?)\*\*/g, '$1')
      .replace(/^\s*[-*]\s+/gm, '• ')
  ...
  async getSummaries(docId: string) {
