// 王者荣耀英雄专属提示词映射
// 用于：1. 生成英雄预览图 2. COS换脸融合生成

export interface HeroPrompt {
  /** 中文名 */
  name: string;
  /** 预览图提示词（纯文生图，用于生成英雄模板预览） */
...
export const HERO_PROMPTS: Record<string, HeroPrompt> = {
  蚩妩: {
    name: '蚩妩',
    previewPrompt:
  ...
  孙悟空: {
    name: '孙悟空',
    previewPrompt:
      '1boy, solo, full body, standing, monkey king, golden armor with dragon engravings, phoenix feather golden cap, fiery golden eyes, wielding golden staff with both hands, battle pose, fire and clouds background, divine warrior, beautiful, anime style, masterpiece, best quality, highly detailed',
    cosPrompt:
      '1boy, cosplay, golden armor, phoenix cap, golden staff, fire clouds, divine warrior, detailed, 8k',
  ...
  韩信: {
    ...
    cosPrompt:
      '1boy, cosplay, silver red armor, dragon spear, fierce expression, battlefield, fire, detailed, 8k',
  ...
  赵云: {
    ...
    cosPrompt:
      '1boy, cosplay, white silver armor, dragon spear, white cape, noble, mountain background, detailed, 8k',
    description: 'Dragon warrior in white',
  },
};

/** 获取英雄提示词，不存在时返回通用提示词 */
export function getHeroPrompt(heroName: string): HeroPrompt {
  return (
    HERO_PROMPTS[heroName] || {
      name: heroName,
