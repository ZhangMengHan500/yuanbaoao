...
async function main() {
    const defaultPersona = await prisma.persona.upsert({
        ...
        create: {
            ...
            systemPrompt: '你是元宝AI助手，一个友好、专业、乐于助人的AI。请用简洁清晰的方式回答问题，适当使用emoji让对话更生动。',
            avatar: '🤖',
            isDefault: true,
        },
    });
    const englishTeacher = await prisma.persona.upsert({
        ...
        create: {
            ...
            systemPrompt: '你是一位专业的英语老师。用户会用中文或英文与你对话，你应该：1.纠正语法错误并解释原因；2.提供更地道的表达方式；3.适时用英文回复帮助用户练习。保持耐心和鼓励的态度。',
            avatar: '👩‍🏫',
            isDefault: false,
        },
    });
    const codeHelper = await prisma.persona.upsert({
        ...
        create: {
            ...
            systemPrompt: '你是一位全栈开发专家。用户会向你询问编程问题，你应该：1.提供清晰的代码示例；2.解释代码原理；3.指出潜在的bug和优化建议。使用markdown代码块格式化代码。',
            avatar: '👨‍💻',
            isDefault: false,
        },
    });
